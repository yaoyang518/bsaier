...
public interface SomkeRepository extends JpaRepository<CityServer, Long> {
    ...
    @Query(value = "SELECT SUM(smoke_time) FROM smoke_daily WHERE createtime>=:start and  createtime<=:end and  boxid in(select boxid from boxs where staus=1 and dotid=:dotid)", nativeQuery = true)
    Integer getSmokeTimeByDotIdAndDate(@Param("dotid") Long dotid, @Param("start") Date start, @Param("end") Date end);

    //报警城市数---------
    @Query(value = "SELECT COUNT(DISTINCT(cityID)) FROM sys_destrict WHERE districtID in(SELECT DISTINCT(districtID) from dot_server WHERE  stid=2 and dotid in(SELECT DISTINCT(dotid) FROM boxs WHERE boxid in (SELECT DISTINCT(boxid) FROM smoke_daily WHERE smoke_time>0)))", nativeQuery = true)
    Integer getSmokeCityCountByStid(@Param("stid") Long stid);

    //报警网点数---------
    @Query(value = "SELECT COUNT(dotid) from dot_server WHERE stid =:stid and dotid in(SELECT DISTINCT(dotid) FROM boxs WHERE boxid in (SELECT DISTINCT(boxid) FROM smoke_daily WHERE smoke_time>0))", nativeQuery = true)
    Integer getSmokeDotCountByStid(@Param("stid") Long stid);

    @Query(value = "SELECT * FROM (SELECT * from dot_server WHERE stid =:stid and dotid in(SELECT DISTINCT(dotid) FROM boxs WHERE boxid in (SELECT DISTINCT(boxid) FROM smoke_daily WHERE smoke_time>0))) as dot JOIN (SELECT * FROM sys_destrict WHERE cityID =:cityId) as dis ON dot.districtID=dis.districtID", nativeQuery = true)
